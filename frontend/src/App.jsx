...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 flex flex-col">
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-4">
            <div className="relative">
              <span className="text-4xl sm:text-5xl animate-bounce" style={{ animationDuration: '2s' }}>🐘</span>
              <span className="absolute -bottom-1 -right-1 text-sm sm:text-lg">🇰🇪</span>
            </div>
            <div>
              <h1 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-emerald-400 via-amber-400 to-red-500 bg-clip-text text-transparent">
                Tembo AI
              </h1>
              <p className="text-slate-400 text-xs sm:text-sm flex items-center gap-2">
                <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
                <span className="hidden xs:inline">Kenya Travel Assistant</span>
                <span className="xs:hidden">Travel Guide</span>
              </p>
            </div>
          </div>
      ...
      {/* Chat Messages */}
      <main className="relative z-10 flex-1 overflow-y-auto px-3 sm:px-4 py-4 sm:py-6">
        <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
          {messages.length === 0 && (
            <div className="text-center py-4 sm:py-8 animate-fade-in">
              {/* Hero Section */}
              <div className="relative inline-block mb-4 sm:mb-6">
                <span className="text-6xl sm:text-8xl block animate-float">🦁</span>
              ...
              <p className="text-slate-400 mb-6 sm:mb-8 text-base sm:text-lg px-4">Discover the magic of Kenya — from savannas to shores</p>
              
              {/* Feature Cards */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-6 sm:mb-8 px-2">
                {[
                  ...
                  <div 
                    key={feature.title}
                    className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-xl sm:rounded-2xl p-3 sm:p-4 hover:bg-slate-800/50 hover:border-emerald-500/30 transition-all hover:scale-105 cursor-pointer group active:scale-95"
                    style={{ animationDelay: `${i * 100}ms` }}
                    onClick={() => handleSuggestion(`Tell me about ${feature.title.toLowerCase()} in Kenya`)}
                  >
                    <span className="text-2xl sm:text-3xl mb-1 sm:mb-2 block group-hover:scale-110 transition-transform">{feature.icon}</span>
                    <h3 className="text-white font-semibold text-sm sm:text-base">{feature.title}</h3>
                    <p className="text-slate-500 text-[10px] sm:text-xs hidden sm:block">{feature.desc}</p>
                  </div>
                ))}
              </div>

              {/* Suggestion Chips */}
              <p className="text-slate-500 text-xs sm:text-sm mb-2 sm:mb-3">Try asking about:</p>
          ...
          <form onSubmit={sendMessage} className="flex gap-2 sm:gap-3">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full bg-slate-800/50 text-white placeholder-slate-500 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4 pr-10 sm:pr-12 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 border border-slate-700/50 focus:border-emerald-500/50 transition-all shadow-inner text-sm sm:text-base"
                disabled={isLoading}
              />
              <div className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 text-slate-600 hidden sm:block">
            ...
            <button
              ...
              className="bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 disabled:from-slate-700 disabled:to-slate-600 disabled:cursor-not-allowed text-white px-4 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-medium transition-all flex items-center gap-2 shadow-lg shadow-emerald-500/25 disabled:shadow-none hover:scale-105 active:scale-95"
            >
...
function ChatMessage({ message, isLatest }) {
  ...
  return (
    <div 
      className={`flex items-start gap-2 sm:gap-3 ${isUser ? 'flex-row-reverse' : ''} animate-slide-up`}
    >
      {/* Avatar */}
      <div className={`w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center text-xl sm:text-2xl flex-shrink-0 shadow-lg ${
        isUser 
      ...
      <div className={`max-w-[85%] sm:max-w-[80%] ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        {/* Bubble */}
        <div className={`rounded-xl sm:rounded-2xl px-3 sm:px-5 py-3 sm:py-4 shadow-xl ${
          isUser 
          ...
          {!isComplete && isLatest && message.animate && !isUser && (
            <span className="inline-block w-2 h-4 sm:h-5 bg-emerald-400 ml-1 animate-pulse rounded-sm"></span>
          )}
        </div>

        {/* Sources */}
        {message.sources && message.sources.length > 0 && isComplete && (
          <div className="mt-2 sm:mt-3 flex flex-wrap gap-1.5 sm:gap-2 animate-fade-in">
            {message.sources.slice(0, 3).map((source, i) => (
              <span 
                key={i} 
                className="text-[10px] sm:text-xs bg-slate-800/60 text-slate-400 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full flex items-center gap-1 sm:gap-1.5 border border-slate-700/50 hover:border-emerald-500/30 transition-colors"
              >
                <span className="text-emerald-400">📍</span>
                <span className="truncate max-w-[80px] sm:max-w-none">{source.destination}</span>
...
function LoadingIndicator() {
  return (
    <div className="flex items-start gap-2 sm:gap-3 animate-slide-up">
      <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center text-xl sm:text-2xl shadow-lg shadow-emerald-500/25">
        🐘
      </div>
      <div className="bg-gradient-to-br from-slate-800/90 to-slate-800/70 rounded-xl sm:rounded-2xl rounded-tl-sm px-3 sm:px-5 py-3 sm:py-4 border border-slate-700/50">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex gap-1">
            {[0, 1, 2].map(i => (
              <span 
                key={i}
                className="w-2 h-2 sm:w-3 sm:h-3 bg-emerald-500 rounded-full animate-bounce"
